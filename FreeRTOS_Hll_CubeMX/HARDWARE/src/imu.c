...
#define MPU_HSPI SPI5
#define MPU_NSS_LOW   LL_GPIO_ResetOutputPin(GPIOF, GPIO_PIN_6)
#define MPU_NSS_HIGH  LL_GPIO_SetOutputPin(GPIOF, GPIO_PIN_6)
...
uint8_t mpu_write_byte(uint8_t const reg, uint8_t const data)
{
    ...
    MPU_NSS_LOW;
    tx = reg & 0x7F;
    SPI5_Read_Write_Byte(tx);
    tx = data;
    status = SPI5_Read_Write_Byte(tx);
    MPU_NSS_HIGH;
    return status;
}
...
uint8_t mpu_read_bytes(uint8_t const regAddr, uint8_t* pData, uint8_t len)
{
    ...
    SPI5_Read_Write_Byte(tx);
		for (;len>0;len--)
		{
			pData[len_tx] = SPI5_Read_Write_Byte(tx);
			len_tx++;
		}
    MPU_NSS_HIGH;
    return 0;
